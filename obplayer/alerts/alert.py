...
class ObAlert(object):
    ...
    def parse_cap_xml(self, xmlcode):
        ...
        for alert in alerts.getElementsByTagName("alert"):
            ...
            self.references = [
                ref.split(",")
                for ref in xml_get_first_tag_value(
                    alert, "references", default=""
                ).split()
            ]
            self.info = []
    ...
    def generate_audio(self, language, voice=None, indigenous=False):
        ...
        if indigenous == True:
            ...
            cg_message_text = self.get_first_info(
                "english", bestmatch=True
            ).get_message_text(False)
        if indigenous == False:
            truncate = not self.broadcast_immediately() and obplayer.Config.setting(
                "alerts_truncate"
            )
        ...
        if indigenous == False:
            ...
            if resources:
                ...
                self.write_tts_file(
                    os.path.join(location, filename), message_text, voice
                )
            else:
                return False
        else:
            self.write_indigenous_file(
                os.path.join(location, filename),
                self.get_first_info("indigenous", bestmatch=True).get_message_text(
                    False, True
                ),
            )
            # over writting for cg scroll text and logging. TODO: shoud be indigenous text here.
        ...
        if resources:
            filename = (
                self.reference(self.sent, self.identifier) + "-" + language + ".jpg"
            )
            if resources[0].write_file(os.path.join(location, filename)) is False:
                return False
    ...
    def get_media_info(
        ...
        secondary_info = (
            self.media_info[secondary_language] if secondary_language else None
        )
                ...
                with wave.open(message, "rb") as read_only_wf:
                    ...
                    if sample_width != 2:
                        process = subprocess.Popen(
                            ["sox", message, "-b", "16", "/tmp/" + "audio.wav"]
                        )
                        process.wait()
                        if process.poll() == 0:
                            ...
                            obplayer.Log.log(
                                "sox returned a error, or a isn't installed.", "error"
                            )
                            pass
    ...
    def write_tts_file(self, path, message_text, voice=None):
        ...
        if voice.startswith("aws"):
            ...
            polly_client = boto3.Session(
                aws_access_key_id=obplayer.Config.setting("aws_access_key_id"),
                aws_secret_access_key=obplayer.Config.setting("aws_secret_access_key"),
                region_name=obplayer.Config.setting("aws_region_name"),
            ).client("polly")
            ...
            try:
                response = polly_client.synthesize_speech(
                    VoiceId=voice,
                    OutputFormat="pcm",
                    Text='<speak><prosody volume="+3dB"><break time="1s" /> '
                    + message_text
                    + "</prosody></speak>",
                    TextType="ssml",
                )
                ...
                obplayer.Log.log(
                    "AWS error such as network outage, or invaild aws ids/keys in use. error: {0}\nUsing local tts as backup audio.",
                    "error".format(e),
                )
                voice = "en"
                proc = subprocess.Popen(
                    [
                        ...
                        "140",
                        "-a",
                        str(obplayer.Config.setting("alerts_voice_volume")),
                        "--stdout",
                    ],
    ...
    def get_indigenous_languages_by_sgcs(sgcs):
        ...
        for sgc in sgcs:
            if type(sgc) != str:
                ...
                continue
            if (
                str(sgc).startswith("47")
                or str(sgc).startswith("48")
                or str(sgc).startswith("59")
                or str(sgc).startswith("61")
                and "Cree" not in languages
            ):
            ...
            if (
                str(sgc).startswith("46")
                or str(sgc).startswith("47")
                and "Cree" not in languages
            ):
                if "Cree" not in languages:
                    print("Cree")
            ...
            if (
                str(sgc).startswith("35")
                or str(sgc).startswith("46")
                or str(sgc).startswith("47")
                and "Cree" not in languages
            ):
                if "Cree" not in languages:
            ...
            if (
                str(sgc).startswith("24")
                or str(sgc).startswith("10")
                and "Innu" not in languages
            ):
                languages.append("Innu")
            # Check for Inuktitut
            if (
            ...
            # Check for Mikmaq
            if (
                str(sgc).startswith("12")
                or str(sgc).startswith("13")
                or str(sgc).startswith("11")
                or str(sgc).startswith("10")
...
class ObAlertInfo(object):
    ...
    def get_resources(self, typename=None):
        return [
            ...
            if not typename or typename in resource.mimetype
        ]
    ...
    def get_message_text(self, truncate=False, indigenous=False):
        ...
        if not text or self.language == "indigenous":
            ...
            if self.language == "fr-CA":
                ...
                self.indigenous = ObAlert.get_indigenous_languages_by_sgcs(
                    output_geocodes
                )
                message_text = ""
                self.indigenous_languages_enabled = obplayer.Config.setting(
                    "alerts_selected_indigenous_languages"
                ).split(",")
                for indigenous in self.indigenous:
                    # check if language is enabled.
                    if (
                        ObAlert.lang_ref(indigenous.lower())
                        in self.indigenous_languages_enabled
                    ):
                        print("lang_ref:", ObAlert.lang_ref(indigenous))
                        # Check to make sure file exists.
                        if os.path.isfile(
                            "{0}/indigenous/{1}/{2}.wav".format(
                                obplayer.Config.datadir,
                                indigenous.lower(),
                                self.event.lower(),
                            )
                        ):
        ...
        if self.language == "indigenous":
            ...
            for indigenous_language in self.indigenous:
                if (
                    ObAlert.lang_ref(indigenous_language.lower())
                    in self.indigenous_languages_enabled
                ):
                    message_text += "{0}/indigenous/{1}/{2}.wav\n".format(
                        obplayer.Config.datadir,
                        indigenous_language.lower(),
                        self.event.lower(),
                    )
            text = message_text
        if self.language != "indigenous":
        ...
        if self.geocodes != None:
            for geocode in self.geocodes:
                if (
                    geocode[0] == "SAME"
                    or geocode[0] == "FIPS6"
                    or geocode[0] == "profile:CAP-CP:Location:0.3"
                ):
        ...
        for geocode in self.geocodes:
            for code in codes:
                ...
                if obplayer.Config.setting("alerts_location_type") == "US":
                    ...
                    if geocode[0] == "profile:CAP-CP:Location:0.3" and (
                        geocode[1].startswith(code) or code.startswith(geocode[1])
                    ):
                        return True
        return False
    ...
    def parse_resource(self, resource):
        ...
        if derefuri is not None:
            ...
            if digest != calculated_digest:
                obplayer.Log.log(
                    "emergency alert resource corrupted: " + str(self.uri), "error"
                )
            else:
                try:
                    ...
                    obplayer.Log.log(
                        "error parsing base64 resource from emergency alert", "error"
                    )
            ...
            try:
                ...
                if r.status_code == requests.codes.ok:
                    ...
                    obplayer.Log.log(
                        "error fetching alert resource %s: returned status code %s"
            ...
            # except requests.exceptions.RequestException:
                ...
                obplayer.Log.log(
                    "connection error while fetching alert resource %s" % (self.uri,),
                    "alerts",
                )
