...
class LEDSignTrigger(object):
    ...
    def sign_write_message(self):
        try:
            obplayer.Log.log(
                "sent message to LED sign on serial port " + self.sign_serial_file,
                "alerts",
            )
            if self.trigger_serial_sign:
                self.trigger_serial_sign.close()

            self.trigger_serial_sign = serial.Serial(
                self.sign_serial_file, baudrate=9600
            )
            self.trigger_serial_sign.write(
            ...
            obplayer.Log.log(
                "failed to send message LED sign on serial port "
                + self.sign_serial_file,
                "alerts",
            )
            obplayer.Log.log(traceback.format_exc(), "error")
    ...
    def sign_clear_message(self):
        try:
            obplayer.Log.log(
                "sent clear to LED sign on serial port " + self.sign_serial_file,
                "alerts",
            )
            if self.trigger_serial_sign:
                self.trigger_serial_sign.close()
