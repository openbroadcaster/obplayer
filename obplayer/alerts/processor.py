...
class ObAlertPullFetcher(obplayer.ObThread):
    ...
    def poll_server(self):
        while True:
            headers = {"user-agent": "OBPlayer", "accept": "application/atom+xml"}
            ...
            if r.status_code == 200:
                ...
                obplayer.Log.log(
                    "Got error code %s polling CAP Server with URL: %s"
            ...
            try:
                ...
                if data:
                    ...
                    with open(
                        ...
                        + "/alerts/"
                        + obplayer.alerts.ObAlert.reference(
                            alert.sent, alert.identifier
                        )
                        + ".xml",
                        "wb",
                    ) as f:
...
class ObAlertTCPFetcher(ObAlertFetcher):
    ...
    def connect(self):
        ...
        for urlstring in self.hosts:
            ...
            (self.host, self.port) = (
                urlparts[0],
                urlparts[1] if len(urlparts) > 1 else 80,
            )
            self.socket = None
            try:
            ...
            if self.socket is not None:
                obplayer.Log.log(
                    "connected to alert broadcaster at "
                    + str(self.host)
                    + ":"
                    + str(self.port),
                    "alerts",
            ...
            obplayer.Log.log(
                "error connecting to alert broadcaster at "
                + str(self.host)
                + ":"
                + str(self.port),
                "error",
...
class ObAlertProcessor(object):
    def __init__(self):
        ...
        self.streaming_hosts = [
            obplayer.Config.setting("alerts_naad_stream1"),
            obplayer.Config.setting("alerts_naad_stream2"),
        ]
        self.archive_hosts = [
            obplayer.Config.setting("alerts_naad_archive1"),
            obplayer.Config.setting("alerts_naad_archive2"),
        ]
        self.target_geocodes = [
            geocode.strip()
            for geocode in obplayer.Config.setting("alerts_geocode").split(",")
        ]
        ...
        if (
            obplayer.Config.setting("alerts_location_type") == "US"
            and obplayer.Config.setting("alerts_language_primary") == "en-CA"
        ):
        ...
        if (
            obplayer.Config.setting("alerts_location_type") == "US"
            and obplayer.Config.setting("alerts_language_secondary") == "en-CA"
        ):
            self.language_secondary = "en-US"
        else:
            self.language_secondary = obplayer.Config.setting(
                "alerts_language_secondary"
            )
        ...
        self.ctrl = obplayer.Player.create_controller(
            "alert", priority=100, default_play_mode="overlap", allow_overlay=True
        )
    ...
    def get_alerts(self):
        alerts = {
            ...
            "next_play": self.next_alert_check,
        }
        with self.lock:
            for name, alert_list in [
                ("active", self.alerts_active),
                ("expired", self.alerts_expired),
            ]:
    ...
    def handle_dispatch(self, alert):
        ...
        if not seen and alert.status == "system":  # or alert.msgtype == 'update':
            self.fetch_references(
                alert.references, required=True if alert.status == "system" else False
            )
        ...
        if alert.msgtype in ("update", "cancel"):
            for _, identifier, _ in alert.references:
                if identifier in self.alerts_active:
                    alert.previously_important = self.alerts_active[
                        identifier
                    ].broadcast_immediately()  # message updates might not have the BI flag set
                    self.mark_expired(self.alerts_active[identifier])
    ...
    def fetch_reference(self, sender, identifier, timestamp, required=False):
        ...
        for host in self.archive_hosts:
            ...
            try:
                obplayer.Log.log(
                    "fetching alert %s using url %s" % (identifier, url), "debug"
                )
                headers = {
                ...
                if r.status_code == 200:
                    # r.encoding = 'utf-8'
                    with open(
                        obplayer.ObData.get_datadir() + "/alerts/" + filename + ".xml",
                        "wb",
                    ) as f:
                        f.write(r.content)
                ...
                pass
        obplayer.Log.log(
            "error fetching alert %s" % (identifier,), "error" if required else "debug"
        )
    ...
    def sort_by_importance(self, alerts):
        return sorted(
            alerts,
            key=lambda alert: alert.received_at
            * (10000 if alert.broadcast_immediately() else 1),
            reverse=True,
        )

    def run(self):
        self.next_purge_check = (
            time.time() if obplayer.Config.setting("alerts_purge_files") else None
        )
        ...
        while not self.thread.stopflag.wait(1):
            try:
                ...
                if (
                    self.next_purge_check is not None
                    and present_time > self.next_purge_check
                ):
                    ...
                    for filename in os.listdir(basedir):
                        try:
                            ...
                            if filedate < then:
                                obplayer.Log.log(
                                    "deleting alert file " + filename, "debug"
                                )
                ...
                if present_time > self.next_alert_check:
                    if len(self.alerts_active) > 0:
                        obplayer.Log.log(
                            "playing active alerts (%d alert(s) to play)"
                            % (len(self.alerts_active),),
                            "player",
                        )  # moved from alerts
                        ...
                        self.ctrl.add_request(
                            media_type="alert",
                            mixerstart="alert_on",
                            title="alert lead in delay",
                            duration=self.leadin_delay,
                            onstart=self.trigger_alert_cycle_start,
                        )
                        ...
                        with self.lock:
                            ...
                            for alert in self.sort_by_importance(
                                self.alerts_active.values()
                            ):
                                ...
                                if obplayer.Config.setting(
                                    "alerts_broadcast_message_in_indigenous_languages"
                                ):
                                    alert_media = alert.get_media_info(
                                        self.language_primary,
                                        self.voice_primary,
                                        self.language_secondary,
                                        self.voice_secondary,
                                        True,
                                    )
                                    if alert.times_played == 0:
                                        obplayer.Log.log(
                                            "playing alert: {0}".format(
                                                alert.identifier
                                            ),
                                            "player",
                                        )  # moved from alerts
                                        obplayer.alert_counter.Alert_Counter().add_alert(
                                            alert.identifier, alert.alert_type
                                        )
                                        obplayer.Log.log(
                                            "playing alert media: {0}".format(
                                                alert_media
                                            ),
                                            "player",
                                        )  # moved from alerts
                                ...
                                obplayer.Log.log(
                                    "changed number of plays for alert: {0} to {1}".format(
                                        alert.identifier, alert.times_played
                                    ),
                                    "alerts",
                                    alert,
                                )
                                if (
                                    os.path.isfile(
                                        obplayer.Config.setting(
                                            "alerts_alert_start_audio"
                                        )
                                    )
                                    and obplayer.Config.setting(
                                        "alerts_play_leadin_enable"
                                    )
                                    == True
                                ):
                                    if (
                                        ...
                                        mediainfo = d.discover_uri(
                                            obplayer.Player.file_uri(
                                                obplayer.Config.setting(
                                                    "alerts_alert_start_audio"
                                                )
                                            )
                                        ...
                                        if (
                                            mediainfo.get_duration() / float(Gst.SECOND)
                                        ) > 10.0:
                                            obplayer.Log.log(
                                                "alert start message is longer then 10 seconds! max allowed is 10 seconds. only playing the first 10 seconds.",
                                                "alerts",
                                            )
                                            self.ctrl.add_request(
                                                media_type="alert",
                                                uri=obplayer.Player.file_uri(
                                                    obplayer.Config.setting(
                                                        "alerts_alert_start_audio"
                                                    )
                                                ),
                                ...
                                self.ctrl.add_request(
                                    media_type="alert",
                                    title="alert tone delay",
                                    duration=1.0,
                                )
                                # Play US/EAS attn tone
                                if (
                                    obplayer.Config.setting("alerts_location_type")
                                    == "US"
                                ):
                                    self.ctrl.add_request(
                                        media_type="alert",
                                        uri=obplayer.Player.file_uri(
                                            "obplayer/alerts/data",
                                            "attention-signal.ogg",
                                        ),
                                        duration=8,
                                        artist=alert_media[0]["audio"]["artist"],
                                        title=alert_media[0]["audio"]["title"],
                                        overlay_text=alert_media[0]["audio"][
                                            "overlay_text"
                                        ],
                                    )
                                # Play CA/Alert Ready attn tone
                                    ...
                                    self.ctrl.add_request(
                                        media_type="alert",
                                        uri=obplayer.Player.file_uri(
                                            os.path.expanduser("~/.openbroadcaster"),
                                            "attn.wav",
                                        ),
                                        duration=8,
                                        artist=alert_media[0]["audio"]["artist"],
                                ...
                                for mediainfo in alert_media:
                                    if "audio" in mediainfo:
                                        mediainfo["audio"][
                                            "media_type"
                                        ] = "alert"  # now using alert pipeline
                                        self.ctrl.add_request(**mediainfo["audio"])
                                    if "visual" in mediainfo:
                                        self.ctrl.add_request(
                                            start_time=start_time, **mediainfo["visual"]
                                        )
                                ...
                                if (
                                    self.repeat_times > 0
                                    and alert.times_played >= self.repeat_times
                                ) or (
                                    alert.max_plays > 0
                                    and alert.times_played >= alert.max_plays
                                ):
                                    expired_list.append(alert)
                        ...
                        self.ctrl.add_request(
                            media_type="alert",
                            mixerend="alert_off",
                            title="alert lead out delay",
                            duration=self.leadout_delay,
                            onend=self.trigger_alert_cycle_stop,
                        )
                    ...
                    self.next_alert_check = self.ctrl.get_requests_endtime() + (
                        self.repeat_interval * 60
                    )
                ...
                if obplayer.Config.setting("alerts_location_type") == "CA":
                    if (
                        self.fetcher.last_received
                        and time.time() - self.fetcher.last_received > 360
                    ):
                        obplayer.Log.log(
                            "no heartbeat received for 6 min. resetting alert fetcher",
                            "error",
                        )
                        ...
                        if self.fetcher_timeouts > 10:
                            os.system(
                                "play -q -n synth 0.8 sin 880; sleep 1; play -q -n synth 0.8 sin 880; sleep 1; play -q -n synth 0.8 sin 880; sleep 1; play -q -n synth 0.8 sin 880"
                            )
                            # resetting timeouts count.
                            obplayer.Log.log(
                                "no heartbeat received timedout. alerting user!",
                                "error",
                            )
                            self.fetcher_timeouts = 0
                        self.fetcher.close()
                ...
                obplayer.Log.log(
                    "exception in " + self.thread.name + " thread", "error"
                )
                obplayer.Log.log(traceback.format_exc(), "error")
