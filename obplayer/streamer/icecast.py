...
class ObIcecastStreamer(ObGstStreamer):
    def __init__(
        self,
        streamer_icecast_ip,
        streamer_icecast_port,
        streamer_icecast_username,
        streamer_icecast_password,
        streamer_icecast_mount,
        streamer_icecast_streamname,
        streamer_icecast_description,
        streamer_icecast_url,
        streamer_icecast_public,
        streamer_icecast_bitrate,
        title_streaming_mode=False,
        streamer_media_mode="audio",
    ):
        ...
        if self.mode == "audio":
            if title_streaming_mode:
                self._metadata_updater_thread = MetadataUpdater(
                    host=self.icecast_ip,
                    port=self.icecast_port,
                    username=self.icecast_username,
                    password=self.icecast_password,
                    mount=self.icecast_mount,
                    mode=title_streaming_mode,
                )
    ...
    def make_video_pipe(self, res):
        ...
        caps.set_property(
            "caps",
            Gst.Caps.from_string(
                "video/x-raw,width={0},height={1},framerate=24/1,pixel-aspect-ratio=1/1".format(
                    video_res.split("x")[0], video_res.split("x")[1]
                )
            ),
        )
