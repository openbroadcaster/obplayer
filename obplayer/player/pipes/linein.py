...
class ObLineInPipeline(ObGstPipeline):
    ...
    def __init__(self, name, player):
        ...
        if self.log:
            ...
            self.logbin_sinkpad = Gst.GhostPad.new(
                "sink", self.logtee.get_static_pad("sink")
            )
    ...
    def msg_detect_silence(self, bus, message, *args):
        ...
        if peak[0] < float(obplayer.Config.setting("audio_in_threshold")):
            if not self.is_silent:
                obplayer.Log.log(
                    self.name
                    + ": silence detected (threshold: "
                    + str(obplayer.Config.setting("audio_in_threshold"))
                    + " dB, disable time: "
                    + str(obplayer.Config.setting("audio_in_disable_time"))
                    + "s)",
                    "debug",
                )
                self.is_silent = True
            ...
            if self.is_silent:
                obplayer.Log.log(
                    self.name
                    + ": audio detected (threshold: "
                    + str(obplayer.Config.setting("audio_in_threshold"))
                    + " dB, enable time: "
                    + str(obplayer.Config.setting("audio_in_enable_time"))
                    + "s)",
                    "debug",
                )
                obplayer.Log.log(
        ...
        if (
            ...
            and (time.time() - self.last_change) > wait_time
        ):
            self.last_change = None
            if self.silence_callback:
                self.silence_callback(self.is_silent)
