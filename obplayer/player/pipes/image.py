...
class ObImagePipeline(ObGstPipeline):
    ...
    def __init__(self, name, player, audiovis=False):
        ...
        self.images_transitions_enable = obplayer.Config.setting(
            "images_transitions_enable"
        )
        ...
        caps.set_property(
            ...
            Gst.Caps.from_string(
                ...
                % (self.images_width, self.images_height, self.images_framerate)
            ),
        )
        ...
        if self.images_transitions_enable:
            ...
            binding = GstController.DirectControlBinding.new(
                self.elements[-1], "alpha", self.control_source
            )
            self.elements[-1].add_control_binding(binding)

            self.elements.append(
                Gst.ElementFactory.make("videomixer", name + "-videomixer")
            )
