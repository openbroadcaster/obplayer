...
class ObRTPInputPipeline(ObGstPipeline):
    ...
    def __init__(self, name, player):
        ...
        if encoding == "OPUS":
            ...
            obplayer.Log.log(
                "invalid encoding format " + str(encoding) + " for RTP input", "error"
            )
        ...
        self.udpsrc_rtp.set_property(
            ...
            Gst.Caps.from_string(
                "application/x-rtp,payload=96,media=audio,channels=2,clock-rate="
                + str(obplayer.Config.setting("rtp_in_clock_rate"))
                + ",encoding-name="
                + str(encoding)
            ),
        )
