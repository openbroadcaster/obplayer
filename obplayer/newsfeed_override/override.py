...
class Override_Thread(obplayer.ObThread):
    def __init__(self):
        ...
        self.ctrl = obplayer.Player.create_controller(
            "news_feed_override", priority=98, allow_requeue=False
        )
        ...
        while self.running:
            for event in self.i.event_gen(yield_nones=False):
                ...
                if not "IN_CLOSE_WRITE" in event[1]:
                    ...
                    obplayer.Log.log(
                        "playing news feed media file: {0}".format(filename),
                        "newsfeed_override",
                    )
                    self.ctrl.add_request(
                        media_type="audio", uri=uri, duration=duration
                    )
                    time.sleep(duration)
                    obplayer.Log.log(
                        "removing news feed media file: {0}".format(filename),
                        "newsfeed_override",
                    )
                    try:
                        ...
                        obplayer.Log.log(
                            "an exception occurred while removing news feed media file: {0}".format(
                                filename
                            ),
                            "newsfeed_override",
                        )
