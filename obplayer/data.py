...
class ObData(object):
    ...
    def set_datadir(cls, name):
        ...
        for language in indigenous_languages:
            if (
                os.access(cls.datadir + "/indigenous/{0}".format(language), os.F_OK)
                == False
            ):
...
class ObConfigData(ObData):
    ...
    def __init__(self):
        ...
        if not self.table_exists("settings"):
            self.execute(
                "CREATE TABLE settings (id INTEGER PRIMARY KEY, name TEXT, value TEXT, type TEXT)"
            )
    ...
    def validate_setting(self, setting_name, setting_value, settings=None):
        ...
        if setting_name == "streamer_icecast_bitrate" and (
            self.is_int(setting_value) == False
            or int(setting_value)
            not in [
                0,
                8,
                16,
        ...
        if setting_name == "offair_audiolog_icecast_bitrate" and (
            self.is_int(setting_value) == False
            or int(setting_value)
            not in [
                0,
        ...
        if (
            setting_name == "sync_url"
            and setting_value
            and url_regex.match(setting_value) == None
        ):
            return "sync_url_invalid"

        if (
            setting_name == "alerts_naad_stream1"
            and url_regex.match(setting_value) == None
        ):
            return "alerts_naad_stream1_invalid"

        if (
            setting_name == "alerts_naad_stream2"
            and url_regex.match(setting_value) == None
        ):
            return "alerts_naad_stream2_invalid"

        if (
            setting_name == "alerts_naad_archive1"
            and url_regex.match(setting_value) == None
        ):
            return "alerts_naad_archive1_invalid"

        if (
            setting_name == "alerts_naad_archive2"
            and url_regex.match(setting_value) == None
        ):
        ...
        if (
            setting_name == "alerts_geocode"
            and geocode_regex.match(setting_value) == None
        ):
        ...
        if (
            setting_name == "fallback_media"
            and os.access(setting_value, os.F_OK) == False
        ):
            return "fallback_media_invalid"

        if (
            setting_name == "local_media"
            and settings["sync_mode"] != "remote"
            and setting_value != ""
            and os.access(setting_value, os.F_OK) == False
        ):
        ...
        if (
            setting_name == "http_admin_secure"
            and int(setting_value)
            and os.access(settings["http_admin_sslcert"], os.F_OK) == False
        ):
        ...
        if (
            setting_name == "live_assist_port"
            and settings["live_assist_enable"]
            and settings["live_assist_port"] == settings["http_admin_port"]
        ):
            return "live_assist_port_invalid"

        lat_regex = re.compile(r"^[-+]?([1-8]?\d(\.\d+)?|90(\.0+)?)$")
        if (
            setting_name == "location_latitude"
            and lat_regex.match(setting_value) == None
        ):
            return "location_latitude_invalid"

        lng_regex = re.compile(r"[-+]?(180(\.0+)?|((1[0-7]\d)|([1-9]?\d))(\.\d+)?)$")
        if (
            setting_name == "location_longitude"
            and lng_regex.match(setting_value) == None
        ):
            return "location_longitude_invalid"

        streams_regex = re.compile(r"^\s+$")
        if (
            setting_name == "station_override_monitored_streams"
            and streams_regex.match(setting_value)
        ):
            return "station_override_monitored_streams_invalid"

        if (
            setting_name == "station_override_server_admin_username"
            and "station_override_server_admin_username" == ""
        ):
    ...
    def check_defaults(self):
        ...
        self.add_setting("streamer_rtmp_mode", "720p", "text")
        self.add_setting("streamer_rtmp_framerate", "30", "text")
        self.add_setting("streamer_rtmp_bitrate", "7500", "int")
        self.add_setting("streamer_rtmp_audio_bitrate", "160", "int")
        self.add_setting("streamer_rtmp_preset_collection", "none", "text")
        ...
        self.add_setting(
            "station_override_monitored_streams", "http://localhost:8000/local", "text"
        )
        ...
        self.add_setting(
            ...
            self.datadir + "/media/" + "L/" + "leadin_message.mp3",
            "text",
        )
        ...
        self.add_setting(
            "alerts_geocode", "10,11,12,13,24,35,46,47,48,59,60,61,62", "text"
        )
        self.add_setting(
            "alerts_broadcast_message_in_indigenous_languages", "0", "bool"
        )
        ...
        self.add_setting(
            "alerts_naad_archive1", "http://capcp1.naad-adna.pelmorex.com", "text"
        )
        self.add_setting(
            "alerts_naad_archive2", "http://capcp2.naad-adna.pelmorex.com", "text"
        )
    ...
    def add_setting(self, name, value, datatype=None):
        ...
        check_setting = self.query(
            "SELECT name,value,type from 'settings' WHERE name = '"
            + self.escape(name)
            + "'"
        )
