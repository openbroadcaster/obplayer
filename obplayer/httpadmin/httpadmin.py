...
class ObHTTPAdmin(httpserver.ObHTTPServer):
    def __init__(self):
        ...
        self.readonly_allow_restart = obplayer.Config.setting(
            "http_readonly_allow_restart"
        )
        ...
        server_address = (
            "",
            obplayer.Config.setting("http_admin_port"),
        )  # (address, port)

        if sslenable:
            httpserver.ObHTTPServer.__init__(
                self,
                server_address,
                sslenable,
                sslreq,
    ...
    def log(self, message):
        ...
        hidden_log_items = [
            "POST /status_info",
            "POST /alerts/list",
            "POST /inter_station_ctrl/start",
            "POST /inter_station_ctrl/stop",
            "POST /inter_station_ctrl/is_live",
        ]
    ...
    def register_routes(self):
        ...
        self.route(
            "/inter_station_ctrl/start", self.req_start_inter_station_ctrl, "admin"
        )
        self.route(
            "/inter_station_ctrl/stop", self.req_stop_inter_station_ctrl, "admin"
        )
        self.route(
            "/inter_station_ctrl/is_live", self.req_is_live_inter_station_ctrl, "admin"
        )
    ...
    def req_status_info(self, request):
        ...
        for stream in requests.keys():
            data[stream] = {
                key: requests[stream][key]
                for key in requests[stream].keys()
        ...
        if "extra" in request.args:
            ...
            if request.args["extra"][0] == "hard":
                try:
                    ...
                    obplayer.Log.log(traceback.format_exc(), "error")
            if (
                request.args["extra"][0] == "hard"
                or request.args["extra"][0] == "defaults"
            ):
    ...
    def req_fstoggle(self, request):
        ...
        if obplayer.Config.headless:
            ...
            return {
                "status": True,
                "fullscreen": "On" if obplayer.Gui.gui_window_fullscreen else "Off",
            }
    ...
    def req_save(self, request):
        if "http_admin_password" in request.args:
            if request.args["http_admin_password"][0] == "":
                ...
                "http_admin_password_retype" not in request.args
                or request.args["http_admin_password"][0]
                != request.args["http_admin_password_retype"][0]
            ):
    ...
    def req_import(self, request):
        ...
        for line in content.split("\n"):
            ...
            if error:
                ...
                settings[name] = value
                obplayer.Log.log(
                    "importing setting '{0}': '{1}'".format(name, value), "config"
                )
    ...
    def req_import_leadin_audio(self, request):
        ...
        if (
            ...
            audio_path = os.path.join(
                ...
                "media",
                "L",
                "leadin_message" + file_type,
            )
            if (
                os.path.exists(
                    os.path.join(obplayer.ObData.get_datadir(), "media", "L")
                )
                == False
            ):
            ...
            for audio_format in ["wav", "mp3", "ogg"]:
                ...
                if (
                    audio_format in format_data
                    and "could not find codec parameters" not in format_data
                ):
                    file_ok = True
    ...
    def req_import_bug_image(self, request):
        ...
        if content != None:
            ...
            ffmpeg_proc = subprocess.Popen(
                ["ffmpeg", "-i", tmp_image_path, "-hide_banner"],
                stdout=subprocess.PIPE,
                stderr=subprocess.PIPE,
            )
            ...
            if file_ok:
                image_path = os.path.join(
                    obplayer.ObData.get_datadir(), "bug." + file_type
                )
    ...
    def req_export(self, request):
        ...
        for name, value in sorted(
            obplayer.Config.list_settings(hidepasswords=True).items()
        ):
            if name != "http_admin_tos_ui_current_tos_text":
                settings += "{0}:{1}\n".format(
                    name, value if type(value) != bool else int(value)
                )
    ...
    def req_update(self, request):
        ...
        proc = subprocess.Popen(
            'cd "' + srcpath + '" && git branch && git pull',
            stdout=subprocess.PIPE,
            shell=True,
        )
    ...
    def req_update_check(self, request):
        ...
        diff = subprocess.Popen(
            'cd "{0}" && git fetch && git diff origin/{1} --quiet'.format(
                srcpath, branchname
            ),
            stdout=subprocess.PIPE,
            shell=True,
        ...
        version = subprocess.Popen(
            'cd "{0}" && git show origin/{1}:VERSION'.format(srcpath, branchname),
            stdout=subprocess.PIPE,
            shell=True,
        )
        ...
        return {
            ...
            "branches": [
                name.strip(" *") for name in branchoutput.decode("utf-8").split("\n")
            ],
        }
    ...
    def req_pulse_volume(self, request):
        ...
        newvol = obplayer.pulse.change_volume(
            request.args["n"][0], request.args["v"][0]
        )
    ...
    def load_strings(lang, strings):
        ...
        for dirname, dirnames, filenames in os.walk(
            os.path.join("obplayer/httpadmin/strings", lang)
        ):
