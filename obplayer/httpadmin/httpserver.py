    ...
    def __init__(
        ...
        if sslenable:
            ...
            sslreq = (
                ssl.CERT_NONE
                if not sslca
                else ssl.CERT_OPTIONAL if not sslreq else ssl.CERT_REQUIRED
            )
            try:
                ...
                obplayer.Log.log(
                    "Error starting OpenSSL server. Falling back to normal HTTP",
                    "error",
                )
...
class ObHTTPRequestHandler(BaseHTTPServer.BaseHTTPRequestHandler):
    ...
    def check_authorization(self):
        ...
        if type(authdata) == str:
            (username, _, password) = (
                base64.b64decode(authdata.split(" ")[-1].encode("utf-8"))
                .decode("utf-8")
                .partition(":")
            )
            ...
            if (
                username == self.server.readonly_username
                and password == self.server.readonly_password
            ):
                self.admin_access = False
    ...
    def do_GET(self):
        ...
        if url.path.startswith("/logs/") or url.path.startswith("/audiologs/"):
            ...
            headers.append(
                (
                    "Content-Disposition",
                    "attachment; filename=" + os.path.basename(unquote(url.path[1:])),
                )
            )
        else:
        ...
        if os.path.isfile(filename):
            ...
            with open(filename, "rb") as f:
                ...
                if self.extension == "html":
                    ...
                    contents = PyHTML(
                        None, getargs, filename, contents.decode("utf-8")
                    ).get_output()
    ...
    def handle_websocket(self, postvars):
        ...
        if protocol.lower() != "audio":
            self.send_content(
                500, "text/plain", "Unrecognized websocket protocol: " + protocol
            )
            return
...
class WebSocketConnection(object):
        ...
        while True:
            ...
            if opcode & WS_OP_CONTROL:
                if opcode == WS_OP_CLOSE:
                    ...
                    obplayer.Log.log(
                        "websocket: recieved ping from " + ":".join(self.client_address)
                    )
                ...
                if msg_opcode == None:
                    ...
                    if opcode != WS_OP_CONT:
                        raise WebSocketError(
                            "websocket: expected CONT opcode, received " + hex(opcode)
                        )
    ...
    def websocket_read_frame(self):
        ...
        if maskkey:
            payload = bytearray(
                ord(b) ^ ord(maskkey[i % 4]) for (i, b) in enumerate(payload)
            )
