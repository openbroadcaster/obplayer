        ...
        while contents != "":
            ...
            segments.append(
                {
                    ...
                    "data": (
                        first[0]
                        if len(first[0]) > 0 and first[0][-1] == "\n"
                        else first[0].rstrip(" \t")
                    ),
                }
            )
            ...
            if second[0]:
                if second[0].startswith("%include"):
                    include = self._read_contents(
                        second[0].replace("%include", "").strip()
                    )
    ...
    def _generate_python(self, segments):
        ...
        for i, seg in enumerate(segments):
            ...
            if seg["type"] == "raw":
                ...
                lines.append(
                    "py._output.write(unicode(eval(py._segments[%d]['data'])))" % (i,)
                )
            elif seg["type"] == "exec":
            ...
            self.write(
                ...
                + html.escape(
                    "\n".join(
                        [
                            str(num + 1) + ":  " + line
                            for num, line in enumerate(self._pycode.splitlines())
                        ]
                    )
                )
                + "</pre>"
