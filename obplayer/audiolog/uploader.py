        ...
        with open(file_path, "rb") as file:
            ...
            if req.status_code == 200:
                ...
                if data["media_supported"] == False:
                    obplayer.Log.log(
                        "Audio log uploaded failed due to a media not supported error. Please enable ogg support in the server.",
                        "audiolog",
                    )
                    return None
                ...
                metadata = {
                    "media": [
                        {
                            "local_id": "2",
                            "artist": "OBPlayer logs",
                            "title": "Station Audio Log ({0} Local)".format(log_date),
                            "album": "Station audio log",
                            ...
                            "advanced_permissions_users": [],
                            "advanced_permissions_groups": [],
                        }
                    ]
                }
                file_data = json.dumps(metadata)
                req = session.post(
                    "{0}/api.php".format(server_url),
                    data={
                        "appkey": obplayer.Config.setting(
                            "audiolog_upload_appkey", True
                        ),
                        "c": "media",
                        "a": "save",
                        "d": file_data,
                ...
                if req.status_code == 200:
                    obplayer.Log.log(
                        "Audio log uploaded at {0}".format(log_date), "audiolog"
                    )
                else:
                    obplayer.Log.log(
                        "Got status code {0} from api call for server media save.".format(
                            req.status_code
                        ),
                        "audiolog",
                    )
                ...
                obplayer.Log.log(
                    "Got status code {0} from api call for server media upload.".format(
                        req.status_code
                    ),
                    "audiolog",
                )
